...
double lerp(double a0, double a1, double w)
{
    ...
    //return (1.0-w) * a0 + w * a1; //linear interpolation
...
double ray_sphere_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    (ray_direction.dot(ray_direction)) * (((ray_origin - sphere_center).dot(ray_origin - sphere_center)) - sphere_radius * sphere_radius);
    if (discrim < 0)
    {
        return -1;
    }
    else
    {
        ...
        double distance = (((-1 * ray_direction).dot((ray_origin - sphere_center))) - sqrt(discrim)) / (ray_direction.dot(ray_direction));
...
double ray_parallelogram_intersection(const Vector3d &ray_origin, const Vector3d &ray_direction, int index, Vector3d &p, Vector3d &N)
{
    ...
    Matrix3d inv_A;
    inv_A << pgram_u, pgram_v, -ray_direction;
    Vector3d B_con = (ray_origin - pgram_origin);
    Vector3d section_check = inv_A.colPivHouseholderQr().solve(B_con);

    if (section_check[2] > 0 && section_check[0] >= 0 && section_check[1] >= 0 && section_check[0] <= 1 && section_check[1] <= 1){
...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    if (nearest_object >= 0 && ((p-ray_origin).norm()) < ((light_position-ray_origin).norm())){
        return false;
    }else{
        return true;
    }
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        p[1] += 1e-5;
        p[2] += 1e-5;
        bool shadow_ray = is_light_visible(p, Li, light_position);
        if (shadow_ray == false){
            continue;
        }
        ...
        //obj_specular_color
        double specular_prep = ((ray_origin - p).normalized() + (light_position - p).normalized()).normalized().dot(N);
        specular_prep = std::max(specular_prep, 0.0);
        specular_prep = pow(specular_prep, obj_specular_exponent);
        const Vector4d specular = obj_specular_color * specular_prep;
    ...
    // use refl_color
    // r = d - 2(d * N)N
    Vector3d view_ray = p-ray_origin;
    Vector3d reflect_dir = (view_ray) - 2 * (view_ray.dot(N)) * N;
    //p + r
    Vector4d reflection_color = refl_color.array() * shoot_ray(p, reflect_dir, max_bounce--).array();
